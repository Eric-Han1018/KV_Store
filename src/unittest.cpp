...
void test_put_small(string db_name)
{
    ...
    assert(db.lsmtree->num_levels == 0);
...
// Test only on a small memtable
// This makes things easier when debugging
void test_get_small(const string& db_name)
{   
    Database db(2);
    db.openDB(db_name);
...
// Test only on a small memtable
// This makes things easier when debugging
void test_scan_small(const string& db_name){
    Database db(6);
    db.openDB(db_name);
...
// Test put with 32MB data + 1MB updates
...
void test_put_big(const string& db_name, Database& db, vector<pair<int64_t, int64_t>>*& memtable_data, vector<pair<int64_t, int64_t>>*& SST_data) {
    ...
    cout << "--- test case 2: Test put() with exceeding tree capacity ---" << endl;
    for (int64_t i = constants::MEMTABLE_SIZE; i < (int64_t)SST_data->size(); ++i) {
        db.put(SST_data->at(i).first, SST_data->at(i).second);
    }
    // Now memtable should be full
    assert(db.memtable->curr_size == db.memtable->memtable_size);

    cout << "--- test case 3: Test Update ---" << endl;
    // Insert same data as in testcase1 while using different keys (-1)
    for (int64_t i = 0; i < constants::MEMTABLE_SIZE; ++i) {
        db.put(SST_data->at(i).first, SST_data->at(i).second);
    }
    // Now memtable should be full
    ...
    // So all previously inserted data shoule be in SSTs
    for (pair<int64_t, int64_t>& entry : *memtable_data) {
        db.put(entry.first, entry.second);
    }
    // Now memtable should be full as well
    assert(db.memtable->curr_size == db.memtable->memtable_size);
}

// Test get() on big data size
void test_get_big(const string& db_name, Database& db, vector<pair<int64_t, int64_t>>*& memtable_data, vector<pair<int64_t, int64_t>>*& SST_data) {
    cout << "--- test case 1: Test get() from memtable ---" << endl;
    const int64_t* value = nullptr;
    for (pair<int64_t, int64_t>& entry : *memtable_data) {
        value = db.get(entry.first, true);
    ...
    for (int64_t i = SST_data->size() * 2; i < (int64_t)SST_data->size() * 2 + 500; ++i) {
        value = db.get(distrib_min(generator), true);
        assert(value == nullptr);
        value = db.get(distrib_max(generator), true);
        assert(value == nullptr);
    }
}

// Test scan() on big data size
void test_scan_big(const string& db_name, Database& db, vector<pair<int64_t, int64_t>>*& memtable_data, vector<pair<int64_t, int64_t>>*& SST_data) {
    ...
    // We test 100 scans, each with incremental range 
    for (int i = 1; i <= 100; ++i) {
    ...
    cout << "--- test case 3: Test scan() with with key1 & key2 out of bound ---" << endl;
    key1 = -443;
    key2 = -100;
    values = db.scan(key1, key2, true);
...
int main(int argc, char **argv) {
    ...
    cout << "\n===== Test Get(key) on small memtable =====\n" << endl;
    test_get_small(db_name);
    cout << "\nTest passed; Now deleting all SSTs & Bloom Filters...\n" << endl;
    deleteSSTs(constants::DATA_FOLDER + db_name);
    cout << "\n===== Test Scan(Key1, Key2) on small memtable =====\n" << endl;
    test_scan_small(db_name);
    cout << "\nTest passed; Now deleting all SSTs & Bloom Filters...\n" << endl;
    deleteSSTs(constants::DATA_FOLDER + db_name);
    ...
    cout << "\nTest passed\n" << endl;
    cout << "\n===== Test Get(key) on big memtable =====\n" << endl;
    test_get_big(db_name, db, memtable_data, SST_data);
    cout << "\nTest passed\n" << endl;
    cout << "\n===== Test Scan(key1, key2) on big memtable =====\n" << endl;
    test_scan_big(db_name, db, memtable_data, SST_data);
    cout << "\nTest passed\n" << endl;
